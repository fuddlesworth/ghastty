...
///
/// Backend-independent (raw CPU pixel data), so it lives at module scope:
/// `Overlay` produces these without needing the graphics API, and the
/// backend-dependent `Image`/`State` (in `Pkg`) consume them.
...
/// Backend-dependent image types (they reference `GraphicsAPI.Texture`
/// and the API's Buffer/Pipeline/RenderPass/shaders). Parameterized over
/// the graphics API so both renderer backends can be compiled together
/// for runtime selection — there is no single global `Renderer.API` to
/// read the backend from anymore. `generic.zig` instantiates this with
/// its `GraphicsAPI`.
pub fn Pkg(comptime GraphicsAPI: type) type {
